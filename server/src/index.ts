...
const PATIENT_COUNT = parseInt(process.env.PATIENT_COUNT || '3000', 10);
...
// Ensure the schema exists and the inventory is seeded.
// Non-destructive: existing data is left untouched. Use `npm run db:reset` for a clean slate.
const initDb = async () => {
  let client;
  try {
...
// Real-time patient vitals stream
...
io.on('connection', (socket: Socket) => {
  ...
  // Send the initial snapshot of all patients
  ...
  const interval = setInterval(() => {
    ...
    for (let i = 0; i < 300; i++) {
      const randomId = Math.floor(Math.random() * PATIENT_COUNT) + 1;
